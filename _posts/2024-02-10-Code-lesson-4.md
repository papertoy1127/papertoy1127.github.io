...
### 목적
<u>IEEE 754</u>는 계산 과정이 <u>하드웨어</u>를 통해 진행되었든 <u>소프트웨어</u>를 통해 진행되었든, 혹은 둘 모두를 통해 진행되었든 동일한 결과를 내도록 <u>부동 소수점 숫자</u>를 계산하기 위한 방법을 제공합니다.

### 포함된 것
<u>IEEE 754</u>는 다음과 같은 것을 명시합니다.
...
### 제외된 것
<u>IEEE 754</u>는 다음과 같은 것을 명시하지 않습니다.
 - 정수의 형식
 - `NaN`에서의 부호부와 가수부의 해석

### 프로그래밍 환경과 고려할 것
<u>IEEE 754</u>는 두 종류의 기수(2, 10)에서 부동 소수점 산술을 명시합니다. 하나의 프로그래밍 환경은 두 기수 중 하나에서, 또는 둘 모두에서 <u>IEEE 754</u>를 따를 수 있습니다.

<u>IEEE 754</u>는 표준을 따르는 프로그래밍 환경의 모든 부분을 정의하지는 않습니다. 이러한 정의되지 않은 동작들은 가능하다면 <u>IEEE 754</u>를 따르는 프로그래밍 언어의 자체적인 정의에서, 그렇지 않다면 특정한 구현에 의해서 정의되는 것이 좋습니다. 어떤 프로그래밍 언어 사양은 특정한 동작들이 (각각의) 구현에 의해서 정의되도록 허용할 가능성이 있습니다.

**언어에 의해 정의된** 동작들은 <u>IEEE 754</u>를 따르는 <u>프로그래밍 언어의 표준</u>을 통해 정의하는 것이 좋습니다. 또한 <u>IEEE 754</u>와 <u>해당 언어 표준</u>을 모두 따르는 구현은 (앞서 정의한) <u>언어에 의해 정의된</u> 동작들에 있어서도 역시 완전히 동일하게 동작해야 합니다. 모든 플랫폼에서 동일한 결과를 내는 것을 목적으로 하는 언어의 표준은 모든 플랫폼에서 성능을 최대한 향상시키는 것을 목적으로 하는 언어의 표준보다 동작들을 더 엄격하게 정의할 것이라고 예상합니다.

<u>IEEE 754</u>는 일반적인 프로그래밍 언어에서 현재 사용할 수 없는 기능들을 필요로 하기 때문에, 몇몇 언어들의 표준은 (그 표준이 개정되기 전까지는) <u>IEEE 754</u>를 완전히 따를 수 없을 가능성이 있습니다. 만약 표준을 따르도록 하기 위해 그러한 언어를 함수 라이브러리나 클래스, 패키지 등으로 확장할 수 있다면, (일반적으로는 <u>프로그래밍 언어의 표준</u>을 통해 정의되는) <u>언어에 의해 정의된</u> 동작들을 그러한 <u>확장을 통해서</u> 정의하는 것이 좋습니다.

**구현에 의해 정의된** 동작들은 <u>IEEE 754</u>를 따르는 프로그래밍 언어의 특정한 <u>구현</u>에 의해서 정의됩니다. 각 구현은 <u>IEEE 754</u>나 <u>프로그래밍 언어의 표준</u>, 프로그래밍 언어의 <u>확장</u>에 속하지 않는 동작을 정의합니다.

<u>IEEE 754</u>를 따른다는 것은 <u>프로그래밍 언어 사양</u> 그 자체가 아니라, 어떤 프로그래밍 환경에서의 <u>특정한 구현</u>의 <u>표준의 준수</u>와 관련된 이야기입니다.

다만 어떠한 <u>프로그래밍 언어 표준</u>에 따른 구현이 모두 자동적으로 <u>IEEE 754</u>를 따르게 설계되었다면, 그 표준을 일컬어 <u>IEEE 754</u>를 따른다고 말할 수 있습니다.

## 형식
<u>IEEE 754</u>를 따르는 모든 형식은 **산술 형식**으로 사용될 수 있습니다.

<u>IEEE 754</u>에 정의되어 있는 특별한 형식 - **교환 형식**의 고정 너비 **인코딩**<a href="#fn-3" id="rfn-3">3</a>은 여러 구현체 사이의 <u>자료 교환</u>에 사용될 수 있습니다. 

<u>IEEE 754</u>에는 다음과 같은 다섯 개의 **기본 형식**이 정의되어 있습니다.
 - 32/64/128비트의 2진법 형식
 - 64/128비트의 10진법 형식

그리고 하술할 추가적인 산술 형식들을 이용하여 기본 형식을 확장할 수 있습니다.
...
| 용어 | 설명 |
|:-------|:-------|
| applicable attribute<br>적용 가능한 속성 | <u>IEEE 754</u>와 관련된 연산 작업의 실행과 관련된 특정한 객체를 제어하는 속성의 값입니다. |
