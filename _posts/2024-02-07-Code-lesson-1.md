...
우리는 숫자를 기억할 때, <U>0-9의 한 자리 숫자</U>를 기준으로 세어갑니다. `4`는 한 자리, `39`는 두 자리, `39800`은 다섯 자리인 것처럼요. 그러니까, 기수법<a href="#fn-1" id="rfn-1" class="fn-link">1</a>으로 **10진법 체계**를 사용한다고 할 수 있겠습니다. 

반면, 컴퓨터가 **2진법 체계**를 사용한다는 것은, 컴퓨터에 관심이 조금 있다면 알려진 상식 정도가 될 것입니다. 2진법은 <U>0과 1</U>만으로 숫자들을 표기하는 기수법입니다. 컴퓨터가 2진법을 사용하는 이유는 간단합니다. 그것이 가장 편리하기 때문이죠. 

컴퓨터는 전기로 작동하는 기계입니다. 앞서 말했던 메모리 역시 전기로 작동합니다. 그런 메모리에서, 이를테면, 10진법을 사용하려고 한다고 해 봅시다. 그렇다면 흐르는 전기의 세기를 <U>10단계로 조절</U>해야 할 것입니다. 그리고 그런 10단계의 전기 세기를 감지하기 위해서도 복잡한 과정이 필요하겠죠.

하지만, 2진법은 매우 간단히 작동합니다. <U>켜지면 1이고, 꺼지면 0</U>입니다. 10개의 세기를 나누어 감지할 필요도 없습니다. 이런 이유로 컴퓨터는 2진법을 사용합니다. 

우리가 10진법에서 숫자의 길이를 <U>몇 자리</U>와 같이 나타내듯이, 컴퓨터에서 2진법 숫자의 각 자리를 **비트**로 나타냅니다. <code>1000<sub>(2)</sub></code>은 4비트, <code>1101 0100<sub>(2)</sub></code>은<a href="#fn-2" id="rfn-2" class="fn-link">2</a> 8비트, 이런 식으로요. 

그러나 2진법에는 단점이 있습니다. 그것은 수를 표기하기 위해서는 너무 길어진다는 것이죠. 10진법에서의 `10`이 2진법으로는 <code>1010<sub>(2)</sub></code>입니다. `2024`는 <code>0111 1110 1000<sub>(2)</sub></code>로 나타내고요. `19721121` 정도가 되면 <code>0001 0010 1100 1110 1011 1010 0001<sub>(2)</sub></code>으로, 굉장히 긴 숫자가 되어버립니다. 이렇게 보이는 것이 길어지는 것도 문제지만, 계산 과정에서도 문제가 발생합니다.

큰 수를 더해본 기억은 누구나 있을 것입니다. 이때, 각 자리의 숫자의 합이 10을 넘어간다면 우리는 받아올림을 합니다. 2진법에서도 마찬가지로, 두 수를 더하는 과정에서 각 자리의 숫자의 합이 2(=10<sub>(2)</sub>)를 넘어간다면 받아올림을 해야 하겠죠. 그런데 여기서 2진법은 각 자릿수가 1 아니면 0이기 때문에 받아올림이 10진법에서보다 훨씬 빈번해지게 됩니다. 또한 위에서 말한 것처럼 자릿수 자체도 늘어나기 때문에, 계산 과정도 복잡해지게 된다고 할 수 있겠죠. 
...
이제 덧셈을 해 봅시다. 덧셈을 하려면 더할 두 개의 숫자가 필요합니다. 우리는 아까 그 숫자들이 이 문서에 써 있었지만, 컴퓨터 입장에서 보자면 메모리밖에 가진 게 없습니다. 그러니까, 메모리에 우리가 더할 숫자를 넣어주면 되겠습니다. 메모리의 1번지에 `01<sub>2</sub>`을 넣고, 2번지에 `10<sub>2</sub>`을 넣어줍시다. 10진법으로 따지면 각각 1과 2가 되겠죠.
...
2진법이든 10진법이든 16진법이든, 모두 공통적으로 포함되는 숫자들이 있기에 혼동의 여지가 있습니다. 이를 좀 더 명확히 하기 위해 앞에 접두어를 붙여 표기하거나, 뒤에 작은 괄호를 표기합니다. 프로그래밍에서는 접두어 방식으로 수를 표기하지만, 본 연재에서는 시각적인 편의를 위해 작은 괄호로 표기하도록 하겠습니다. (다만 종종 접두어 표기를 사용할 일도 있을 테니, 알아 두시기는 하셔야 하겠습니다.)<br><br>
괄호를 표기하는 방식은 간단합니다. 2진법을 <code>1010<sub>(2)</sub></code>, 16진법은 <code>3f9a<sub>(16)</sub></code> 이런 식으로, 작은 괄호 안에 진법을 표기해주는 방식이죠.<br>
접두어는 숫자 앞에 진법을 표기하는 방식입니다. 2진법은 <code><u>0b</u>1010</code>, 16진법은 <code><u>0x</u>3f9a</code>, 그리고 자주 쓰이지는 않지만 8진법은 <code><u>0o</u>3201</code>와 같은 식으로요. 10진법에는 접두어가 따로 없습니다. 이제부터 괄호나 접두어가 붙어있지 않은 숫자가 보인다면, 기본적으로 10진법이라고 생각하셔도 무방하겠습니다.</div>

<div class="footnote" id="fn-2"><a href="#rfn-2">↑</a> <sup>2</sup> 숫자의 길이가 너무 길어져서 네 자리씩 끊어서 표기했습니다. 3980000을 3,980,000으로 끊어 표기하는 것과 비슷하게 생각하시면 됩니다.</div>

<hr>

> 업데이트 24/02/07: 메모리 그림을 이미지에서 표로 바꾸었습니다. 이미지를 하나하나 만들려니까 귀찮더라고요.
{: .prompt-info }

> 업데이트 24/02/08: 문단 제목과 목차를 추가했어요. 움직이기 편하게!
{: .prompt-info }

> 업데이트 24/02/10: 이진법 표기 방식과 그에 대한 설명을 수정했습니다. 0b... 보다는 ...<sub>(2)</sub>가 읽기 편하잖아요?
{: .prompt-info }
