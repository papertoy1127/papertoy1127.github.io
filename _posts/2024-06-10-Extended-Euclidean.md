...
> 여기서는 $a_{k-1} > a_k > 0$인 경우만 생각할 것인데, 이는 $a_{k-1} < a_k$이라면 $\text{GCD}(a_{k-1}, a_k) = \text{GCD}(a_k, a_{k-1}\text{ mod }a_k) = \text{GCD}(a_k, a_{k-1})$이 되기 때문입니다.<br>
...
오늘은 시간이 부족하여 Python 3 코드만이 준비하지 못했습니다. 추후 가능하다면 다른 언어로 된 코드도 올리도록 하겠습니다.
...
다시 위의 식으로 돌아가서, 위 식을 등식으로 바꾸려면 적당한 몫 $q$에 대해서, 
