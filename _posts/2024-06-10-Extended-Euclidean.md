...
**C#**
```cs
(int g, int z, int t) EEA(int a, int b) {
    var (r0, r1, z0, z1, t0, t1) = (a, b, 1, 0, 0, 1);
    while (r1 != 0) {
        var q = r0 / r1;
        (r0, r1, z0, z1, t0, t1) = (r1, r0 - r1*q, z1, z0 - z1*q, t1, t0 - t1*q);
    }
...
마지막에 음수인지 확인하는 코드가 추가된 이유는, C++이나 C# 등의 언어에서는 음수의 나눗셈이 양수의 나눗셈과 약간 다르게 동작하기 때문입니다. (Python에는 해당 문제가 없습니다.)

a와 b가 모두 양수인 게 보장된다면 해당 부분은 생략해도 됩니다.
