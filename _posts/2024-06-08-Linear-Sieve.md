...
## 에라토스테네스의 체
<hr>
[에라토스테네스의 체](https://ko.wikipedia.org/wiki/%EC%97%90%EB%9D%BC%ED%86%A0%EC%8A%A4%ED%85%8C%EB%84%A4%EC%8A%A4%EC%9D%98_%EC%B2%B4)는 $n$ 이하의 소수의 리스트를 $O(n\ log\ log\ n)$ 시간에 구해주는 알고리즘입니다. 아래와 같이 작동하며, 유명한 알고리즘이니만큼 다들 알고 계실 것이라 생각하며 구현은 생략하겠습니다.
...
| $8$   | $2$             | $2 \cdot 8$ |
| $9$   | $9$             | $2 \cdot 9$ | $3 \cdot 9$ |
| $10$  | $2$             | $2 \cdot 10$ |

우리가 방금 했던 것은 체에서 어떤 합성수를 <u>단 한 번만</u> 걸러내기 위한 과정이었습니다. 우리가 체를 수행하는 동안 해야 할 것은 다음과 같습니다.
- $q$를 2부터 쭉 증가시키면서, $\text{spf}(q)$보다 작은 소수 $p$에 대해 $p \cdot q$를 체에서 지운다.

이를 위해 우리에게 필요한 것은 $q$보다 작은 소수들의 리스트입니다. $q$보다 작은 소수들을 순회하다가, $p \mid q$인 순간 $\text{spf}(n) = \text{spf}(q)$가 되기 때문에 멈추면 됩니다.

$q$보다 작은 소수들의 리스트는 어디 있을까요? $q$보다 작은 수 중 체에서 지워지지 않은 수가 바로 $q$보다 작은 소수들이죠! 다만 $q$보다 작은 소수들을 찾겠다고 $1$ ~ $q-1$까지 체를 순회하는 것은 비효율적이니, 미리 리스트를 하나 만들어 놓고 거기에 지워지지 않은 수들을 넣어두면 되겠습니다.
